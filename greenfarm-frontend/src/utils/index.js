...
export function timeHandle(time) {
  ...
  return month + ' ' + date1 + ', ' + year + ' ' + hours + ':' + minutes;
}
...
export function toggleClass(element, className) {
  ...
  if (nameIndex === -1) {
    ...
    const indexName = nameIndex - 1
    const classNameLen = className.length + 1
    classString =
      classString.substr(0, indexName) +
      classString.substr(indexName + classNameLen)
  }
