...
extension SCNNode {
    ...
    func setPivot() {
       ...
       // print("zll--box",maxVec,minVec,maxVec.z - minVec.z)
...
//        self.pivot = SCNMatrix4MakeTranslation(maxVec.x, bound.y, bound.z / 2);
        self.pivot = SCNMatrix4MakeTranslation(maxVec.x+bound.x/7, maxVec.y+bound.x/7, maxVec.z+bound.x/7);
    }
    
    func setPivotArea() {
        let minVec = self.boundingBox.min
        let maxVec = self.boundingBox.max
//        print("zll--boxarea",maxVec,minVec,maxVec.z - minVec.z)
        let bound = SCNVector3Make( maxVec.x - minVec.x, maxVec.y - minVec.y, maxVec.z - minVec.z);
        self.pivot = SCNMatrix4MakeTranslation(bound.x / 2, bound.y, bound.z / 2);
