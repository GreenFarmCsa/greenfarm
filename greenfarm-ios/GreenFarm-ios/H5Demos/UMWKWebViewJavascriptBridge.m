...
@implementation UMWKWebViewJavascriptBridge{
    __weak WKWebView* _webView;
    __weak id<WKNavigationDelegate> _webViewDelegate;
    long _uniqueId;
    UMWebViewJavascriptBridgeBase *_base;
}

/* API
 *****/

+ (void)enableLogging { [UMWebViewJavascriptBridgeBase enableLogging]; }

+ (instancetype)bridgeForWebView:(WKWebView*)webView {
    UMWKWebViewJavascriptBridge* bridge = [[self alloc] init];
...
- (void)send:(id)data responseCallback:(UMJBResponseCallback)responseCallback {
...
- (void) _setupInstance:(WKWebView*)webView {
    ...
    _base = [[UMWebViewJavascriptBridgeBase alloc] init];
    _base.delegate = self;
}
...
- (void)WKFlushMessageQueue {
    [_webView evaluateJavaScript:[_base webViewJavascriptFetchQueyCommand] completionHandler:^(NSString* result, NSError* error) {
        if (error != nil) {
            NSLog(@"UMJSBridge: WARNING: Error when trying to fetch data from WKWebView: %@", error);
        }
